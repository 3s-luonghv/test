	app.controller('ModalDemoCtrl', function ($scope, $uibModalInstance,$uibModal, ModalService, $http, WebService, ValidateUltility, parentParameter) {
	console.log(parentParameter.id);
	 $scope.selectstt = 1 + "";
	var idnext=0;
	if (parentParameter.id > 0) {
		idnext = parentParameter.id;
	...
	function dropdownScreens() {
		...
		WebService.call($http, "screens/screens001", dropdownScreens, function (response) {
			...
			$scope.selectscreens = $scope.screensdropdown[8].id + "";
