app.controller('ModalAddNewCtrl', function($http, $scope,$uibModal, TitleparentParameter, parentParameter, WebService, $uibModalInstance, ValidateUltility, ModalService) {
...
//---------------------------------------------------------------------------//	
	...
	function saveAndContinue(datarequest){
		WebService.call($http, "system/sys05", datarequest, function(response){
			if (response.data.isDone == true ){
				ModalService.alert({"modalTitle":"Success","messenge":"Bạn đã thêm mới thành công"}, function(){}, function(){})
				idnew = response.data.dto.id
			} else {
				ModalService.alert({"modalTitle":"Error","messenge":"Bạn đã thêm mới thất bại"}, function(){}, function(){})
			}
		}, "POST");
	}
	...
	//viết fn để bắt lỗi khi người dùng đã nhập dữ liệu nhưng muốn thoát
	...
	function loadMultiLang() {
		...
		WebService.call($http, "screensitem/screensitem03", dataMultiLang, function(response) {
			...
			$scope.modalsystemtitle = db[0].displayText
			for (x in db) {
